...
void addProd(vector<Producto>&prod) {
	string i,n,c;
	double p, cos;
	int s;
	int y=1;
...
void modificarProd(vector<Producto>&prod){
	if(prod.empty()){
		cout<<"ERROR: No hay productos registrados"<<endl;
	}else{
	string i, n, c;
...
void totalCat(vector<ventas>&vent){
	if(vent.empty()){
		cout<<"ERROR: No hay ventas registradas"<<endl;
	}else{
...
void consultarHist(vector<ventas>&vent, vector<cliente>&cl){
		if(vent.empty()){
			cout<<"ERROR: No hay ventas registradas"<<endl;
		}else{
	string i1;
...
cout<<"El cliente no tiene compras"<<endl;
		...
		if(vent[k].getCliente()==cl[i].getID()){
cout<<"ID de venta: "<<vent[k].getID()<<endl;
			cout<<"Productos: "<<endl;
for(int j=0;j<vent[k].getProd().size();j++){
cout<<vent[k].getProd()[j].getNombre()<<"-"<<vent[k].getProd()[j].getStock()<<" unidades"<<endl;
}
cout<<"==============="<<endl;
				}	
		}
...
void promCateg(vector<Producto>&prod){
	if(prod.empty()){
cout<<"ERROR: No hay productos registrados"<<endl;
	}else{	
...
void cargarProductos(vector<Producto>&prod){
